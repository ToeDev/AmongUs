...
public class SetMaximumCommand {
    ...
    public void execute(final CommandSender sender, String[] args) throws SQLException {
        ...
        if(gameHandler.getPlayersInMapQueue(mapManager.getMap(args[2])) != null && gameHandler.getPlayersInMapQueue(mapManager.getMap(args[2])).size() > Integer.parseInt(args[1])) {
            sender.sendMessage(Prefix.prefix + red + "You cannot lower the maximum players beneath the amount of players already in the queue!");
            return;
        }

        mapManager.setMaxPlayers(args[2].toLowerCase(), Integer.parseInt(args[1]));
        sender.sendMessage(Prefix.prefix + purple + "Maximum players set to " + gold + args[1] + purple + " on map " + gold + args[2]);
    }
